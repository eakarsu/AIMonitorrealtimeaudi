...
// Route: /api/gap-driver
// Feature: Driver
// Description: Driver coaching delivery UI (automated safety training)
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Driver. Driver coaching delivery UI (automated safety training)' },
...
router.post('/', async (req, res) => {
  try {
    ...
    return res.json({ feature: 'driver', title: 'Driver', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'driver', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['driver']);
      return res.json({ slug: 'driver', items: r.rows || [] });
    }
    return res.json({ slug: 'driver', items: [] });
  } catch (e) {
    return res.json({ slug: 'driver', items: [], error: (e && e.message) || 'Server error' });
  }
});
