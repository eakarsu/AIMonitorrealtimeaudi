...
// Route: /api/gap-road-hazard-detector
// Feature: Road Hazard Detector
// Description: (risky intersection identification)
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Road Hazard Detector. (risky intersection identification)' },
...
router.post('/', async (req, res) => {
  try {
    ...
    const prompt = `Feature: Road Hazard Detector\nDescription: (risky intersection identification)\nInput: ${input}\nContext: ${JSON.stringify(context || {})}`;
    const result = await callOpenRouter(prompt);
    // Best-effort persistence
    try {
      if (db && typeof db.query === 'function') {
        await db.query('INSERT INTO gap_features (slug, input, output, meta) VALUES ($1, $2, $3, $4)',
          ['road-hazard-detector', input, JSON.stringify(result), JSON.stringify(context || {})]);
    ...
    return res.json({ feature: 'road-hazard-detector', title: 'Road Hazard Detector', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'road-hazard-detector', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['road-hazard-detector']);
      return res.json({ slug: 'road-hazard-detector', items: r.rows || [] });
    }
    return res.json({ slug: 'road-hazard-detector', items: [] });
  } catch (e) {
    return res.json({ slug: 'road-hazard-detector', items: [], error: (e && e.message) || 'Server error' });
  }
});
