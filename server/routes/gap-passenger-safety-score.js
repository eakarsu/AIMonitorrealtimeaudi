...
// Route: /api/gap-passenger-safety-score
// Feature: Passenger Safety Score
// Description: (per-trip safety rating)
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Passenger Safety Score. (per-trip safety rating)' },
...
router.post('/', async (req, res) => {
  try {
    ...
    const prompt = `Feature: Passenger Safety Score\nDescription: (per-trip safety rating)\nInput: ${input}\nContext: ${JSON.stringify(context || {})}`;
    const result = await callOpenRouter(prompt);
    // Best-effort persistence
    try {
      if (db && typeof db.query === 'function') {
        await db.query('INSERT INTO gap_features (slug, input, output, meta) VALUES ($1, $2, $3, $4)',
          ['passenger-safety-score', input, JSON.stringify(result), JSON.stringify(context || {})]);
    ...
    return res.json({ feature: 'passenger-safety-score', title: 'Passenger Safety Score', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'passenger-safety-score', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['passenger-safety-score']);
      return res.json({ slug: 'passenger-safety-score', items: r.rows || [] });
    }
    return res.json({ slug: 'passenger-safety-score', items: [] });
  } catch (e) {
    return res.json({ slug: 'passenger-safety-score', items: [], error: (e && e.message) || 'Server error' });
  }
});
