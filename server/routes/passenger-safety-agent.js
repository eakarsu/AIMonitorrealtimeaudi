// Auto-generated feature route: passenger-safety-agent
// Domain: passenger safety agent — Capability: providing ETA + emergency assistance
// Mounted under: /api/passenger-safety-agent
...
// POST /api/passenger-safety-agent/run — execute the feature
router.post('/run', async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = 'You are a passenger safety agent. Your task is providing ETA + emergency assistance. Respond with concise JSON: { "summary": string, "recommendations": string[], "next_actions": string[], "confidence": number }.';
...
// GET /api/passenger-safety-agent/history — recent runs
router.get('/history', (req, res) => {
  const limit = Math.min(parseInt(req.query.limit, 10) || 25, 100);
  res.json({ items: store.slice(-limit).reverse() });
});

// GET /api/passenger-safety-agent/:id — fetch one
...
// DELETE /api/passenger-safety-agent/:id — discard a run
