...
// Route: /api/gap-third-party
// Feature: Third Party
// Description: Third-party liability tracking
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Third Party. Third-party liability tracking' },
...
router.post('/', async (req, res) => {
  try {
    ...
    return res.json({ feature: 'third-party', title: 'Third Party', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'third-party', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['third-party']);
      return res.json({ slug: 'third-party', items: r.rows || [] });
    }
    return res.json({ slug: 'third-party', items: [] });
  } catch (e) {
    return res.json({ slug: 'third-party', items: [], error: (e && e.message) || 'Server error' });
  }
});
