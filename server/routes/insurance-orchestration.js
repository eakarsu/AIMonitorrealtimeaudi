// Auto-generated feature route: insurance-orchestration
// Domain: insurance risk orchestrator — Capability: recommending dynamic premiums
// Mounted under: /api/insurance-orchestration
...
// POST /api/insurance-orchestration/run — execute the feature
router.post('/run', async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = 'You are a insurance risk orchestrator. Your task is recommending dynamic premiums. Respond with concise JSON: { "summary": string, "recommendations": string[], "next_actions": string[], "confidence": number }.';
...
// GET /api/insurance-orchestration/history — recent runs
router.get('/history', (req, res) => {
  const limit = Math.min(parseInt(req.query.limit, 10) || 25, 100);
  res.json({ items: store.slice(-limit).reverse() });
});

// GET /api/insurance-orchestration/:id — fetch one
...
// DELETE /api/insurance-orchestration/:id — discard a run
