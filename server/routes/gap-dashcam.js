...
// Route: /api/gap-dashcam
// Feature: Dashcam
// Description: Dashcam video ingest pipeline
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Dashcam. Dashcam video ingest pipeline' },
...
router.post('/', async (req, res) => {
  try {
    ...
    return res.json({ feature: 'dashcam', title: 'Dashcam', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'dashcam', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['dashcam']);
      return res.json({ slug: 'dashcam', items: r.rows || [] });
    }
    return res.json({ slug: 'dashcam', items: [] });
  } catch (e) {
    return res.json({ slug: 'dashcam', items: [], error: (e && e.message) || 'Server error' });
  }
});
