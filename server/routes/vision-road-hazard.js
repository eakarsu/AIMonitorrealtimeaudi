// Auto-generated feature route: vision-road-hazard
// Domain: dashcam road hazard detector — Capability: detecting hazards in dashcam frames
// Mounted under: /api/vision-road-hazard
...
// POST /api/vision-road-hazard/run — execute the feature
router.post('/run', async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = 'You are a dashcam road hazard detector. Your task is detecting hazards in dashcam frames. Respond with concise JSON: { "summary": string, "recommendations": string[], "next_actions": string[], "confidence": number }.';
...
// GET /api/vision-road-hazard/history — recent runs
router.get('/history', (req, res) => {
  const limit = Math.min(parseInt(req.query.limit, 10) || 25, 100);
  res.json({ items: store.slice(-limit).reverse() });
});

// GET /api/vision-road-hazard/:id — fetch one
...
// DELETE /api/vision-road-hazard/:id — discard a run
