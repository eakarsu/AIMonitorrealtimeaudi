...
// Route: /api/gap-distraction-detection
// Feature: Distraction Detection
// Description: (phone use, drowsiness from camera)
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Distraction Detection. (phone use, drowsiness from camera)' },
...
router.post('/', async (req, res) => {
  try {
    ...
    const prompt = `Feature: Distraction Detection\nDescription: (phone use, drowsiness from camera)\nInput: ${input}\nContext: ${JSON.stringify(context || {})}`;
    const result = await callOpenRouter(prompt);
    // Best-effort persistence
    try {
      if (db && typeof db.query === 'function') {
        await db.query('INSERT INTO gap_features (slug, input, output, meta) VALUES ($1, $2, $3, $4)',
          ['distraction-detection', input, JSON.stringify(result), JSON.stringify(context || {})]);
    ...
    return res.json({ feature: 'distraction-detection', title: 'Distraction Detection', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'distraction-detection', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['distraction-detection']);
      return res.json({ slug: 'distraction-detection', items: r.rows || [] });
    }
    return res.json({ slug: 'distraction-detection', items: [] });
  } catch (e) {
    return res.json({ slug: 'distraction-detection', items: [], error: (e && e.message) || 'Server error' });
  }
});
