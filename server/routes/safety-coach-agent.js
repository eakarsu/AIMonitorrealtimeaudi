// Auto-generated feature route: safety-coach-agent
// Domain: agentic driver safety coach — Capability: delivering personalized live guidance to drivers
// Mounted under: /api/safety-coach-agent
...
// POST /api/safety-coach-agent/run — execute the feature
router.post('/run', async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = 'You are a agentic driver safety coach. Your task is delivering personalized live guidance to drivers. Respond with concise JSON: { "summary": string, "recommendations": string[], "next_actions": string[], "confidence": number }.';
...
// GET /api/safety-coach-agent/history — recent runs
router.get('/history', (req, res) => {
  const limit = Math.min(parseInt(req.query.limit, 10) || 25, 100);
  res.json({ items: store.slice(-limit).reverse() });
});

// GET /api/safety-coach-agent/:id — fetch one
...
// DELETE /api/safety-coach-agent/:id — discard a run
