...
// Route: /api/gap-passenger
// Feature: Passenger
// Description: Passenger communication channel (real-time alerts)
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Passenger. Passenger communication channel (real-time alerts)' },
...
router.post('/', async (req, res) => {
  try {
    ...
    return res.json({ feature: 'passenger', title: 'Passenger', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'passenger', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['passenger']);
      return res.json({ slug: 'passenger', items: r.rows || [] });
    }
    return res.json({ slug: 'passenger', items: [] });
  } catch (e) {
    return res.json({ slug: 'passenger', items: [], error: (e && e.message) || 'Server error' });
  }
});
